...
On va créer notre architecture via les librairies **pytorch** et **torchvision** , et celles-ci intègrent des fonctions utiles pour le pré-traitement, comme `transforms` :
...
Nous créons la classe héritée `CoCoDataset` de cette manière :
...
On peut maintenant créer notre instance de classe `dataset` :
```python
#Dataloader
...
On va maintenant pouvoir écrire nos deux réseaux CNN et RNN : 
...
On va finalement pouvoir entrainer notre réseau :
